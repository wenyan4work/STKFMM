...
void stk_ulapu(T *r_src, int src_cnt, T *v_src, int dof, T *r_trg, int trg_cnt,
    ...
    for (int i = 0; i < trg_cnt; i++) {
        ...
        for (int j = 0; j < src_cnt; j++) {
            ...
            vx += fx * invr + dx * fdotr * invr * invr;
            vy += fy * invr + dy * fdotr * invr * invr;
            vz += fz * invr + dz * fdotr * invr * invr;

            lapvx += 2 * fx * invr3 - 6 * fdotr * dx * invr5;
            lapvx += 2 * fx * invr3 - 6 * fdotr * dy * invr5;
            lapvx += 2 * fx * invr3 - 6 * fdotr * dz * invr5;
        }
        v_trg[6 * i + 0] += pi8 * vx;
        v_trg[6 * i + 1] += pi8 * vy;
        v_trg[6 * i + 2] += pi8 * vz;
...
void rpy_u(T *r_src, int src_cnt, T *v_src, int dof, T *r_trg, int trg_cnt,
    ...
    for (int i = 0; i < trg_cnt; i++) {
        ...
        for (int j = 0; j < src_cnt; j++) {
            ...
            vy += a2 * (2 * fx * invr3 - 6 * fdotr * dy * invr5) / 6.0;
            vz += a2 * (2 * fx * invr3 - 6 * fdotr * dz * invr5) / 6.0;
        }
        v_trg[3 * i + 0] += pi8 * vx;
        v_trg[3 * i + 1] += pi8 * vy;
        v_trg[3 * i + 2] += pi8 * vz;
...
inline const Kernel<T> &RPYTestKernel<T>::ulapu() {
    ...
    static Kernel<T> glapg_ker = BuildKernel<T, stk_ulapu<T, NEWTON_ITE>, stk_ulapu<T, NEWTON_ITE>>(
        "stk_ulapu", 3, std::pair<int, int>(3, 6));
    glapg_ker.surf_dim = 3;

    static Kernel<T> gr_ker = BuildKernel<T, rpy_u<T, NEWTON_ITE>, rpy_u<T, NEWTON_ITE>>(
        "rpy_u", 3, std::pair<int, int>(4, 3));
    gr_ker.surf_dim = 4;
    
    static Kernel<T> grlapgr_ker = BuildKernel<T, rpy_ulapu<T, NEWTON_ITE>, rpy_ulapu<T, NEWTON_ITE>>(
